using AutoMapper;
using MediatR;
using SmartExpenseControl.Application.Queries.GetExpensesByGroup;
...
public sealed class GetExpensesHandler(IExpenseRepository expenseRepository, IMapper mapper)
    : IRequestHandler<GetExpensesQuery, PagedResponseOffset<ExpenseSummary>>,
        ...
        IRequestHandler<GetExpensesByGroupQuery, ExpenseSummary>
{
    public Task<PagedResponseOffset<ExpenseSummary>> Handle(GetExpensesQuery request, CancellationToken cancellationToken) =>
        expenseRepository.GetByUserIdAsync(request.UserId, request.Period, request.PageNumber, request.PageSize);
    ...
    public Task<ExpenseSummary> Handle(GetExpensesByGroupQuery request, CancellationToken cancellationToken)
    {
